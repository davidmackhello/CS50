...
char* indexes(const char* path)
{
    // create test strings for path/index.php and path/index.html
    char phpipath[strlen(path) + 9 + 1];
    strcpy(phpipath, path);
    strcat(phpipath, "index.php");
    char htmlipath[strlen(path) + 10 + 1];
    strcpy(htmlipath, path);
    strcat(htmlipath, "index.html");
    
    // check for index.php file in directory
    if (access(phpipath, F_OK) == 0)
    {
        // allocate memory on heap for new path
        char* pindex = malloc((strlen(path) + 9 + 1) * sizeof(char));
        
        // copy string into heap memory
        strcpy(pindex, phpipath);
        
        // return pointer to new path
        return pindex;
    }
    
    // check for index.html file in directory
    else if (access(htmlipath, F_OK) == 0)
    {
        // allocate memory on heap for new path
        char* pindex = malloc((strlen(path) + 10 + 1) * sizeof(char));
        
        // copy string into heap memory
        strcpy(pindex, htmlipath);
        
        // return pointer to new path
        return pindex;
    }
    
    // return NULL if no index file in directory
    else
    {
        return NULL;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    do
    {
        ...
        // reallocate memory for *content with updated size
...
{
    // identify space characters in line, ensuring there are two and only two
    // identify CRLF in line
    char* sp1 = strchr(line, ' ');
    char* sp2 = strchr((sp1 + 1), ' ');
    char* sp3 = strchr((sp2 + 1), ' ');
    char* CRLF = strstr(line, "\r\n");
    
    // return error for lines that do not have two spaces or that do not have CRLF
    if (sp1 == NULL || sp2 == NULL || sp3 != NULL || CRLF == NULL)
    {
        error(400);
        return false;
    }
    
    // extract method element
    char method[sp1 - line + 1];
    strncpy(method, line, (sp1 - line));
    method[sp1 - line] = '\0';
    ...
    // extract request-target element
    char reqtarg[sp2 - sp1];
    strncpy(reqtarg, (sp1 + 1), (sp2 - sp1 - 1));
    reqtarg[sp2 - sp1 - 1] = '\0';
    if (reqtarg[0] != '/')
    {
        error(501);
    ...
    if (strchr(reqtarg, '"') != NULL)
    {
        error(400);
        return false;
    }
    
    // extract HTTP-version element
    char http[CRLF - sp2];
    strncpy(http, (sp2 + 1), (CRLF - sp2 - 1));
    http[CRLF - sp2 - 1] = '\0';
    ...
    // identify '?' character
    char* qmark = strchr(reqtarg, '?');
    
    // store absolute-path and query if no '?' present
    if (qmark == NULL)
    {
        strcpy(abs_path, reqtarg);
        *query = '\0';
        return true;
    }

    //store absolute-path and query if '?' present
    else
    {
        strncpy(abs_path, reqtarg, (qmark - reqtarg));
        int n = qmark - reqtarg;
        *(abs_path + n) = '\0';
    ...
    // respond with error if general problems with request-line
    error(400);
    return false;
}
