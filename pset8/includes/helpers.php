<?php
    ...
    /**
     * Returns formatted string for first argument of CS50 query INSERT function
     */
    function queryarg($tablename, $columns = [])
    {
        // format beginning of string with array of column names
        $string = "INSERT INTO $tablename (";
        $string .= implode(", ", $columns);
        $string .= ") VALUES(";
        
        // add placeholder '?' for each column to be inserted and return string
        $n = count($columns);
        for ($i = 0; $i < ($n - 1); $i++)
        {
            $string .= "?, ";
